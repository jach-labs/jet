...
FAMILIES=['banking','finance','sarcasm']
def family(source):
 return source.split(':')[1] if source.startswith('focused:') else 'retention'
...
def eligible(scores,baseline):
 return all(scores['families'][k]['metric'] >= baseline['families'][k]['metric']-.02 for k in FAMILIES+['retention'])
def main():
 ...
 torch.set_num_threads(4);torch.cuda.set_per_process_memory_fraction(.88);seed=260925
 torch.manual_seed(seed);np.random.seed(seed);random.seed(seed)
 model,tok,info=load_model(str(ROOT/'releases/jet-v6.1'),'f446b82727be57da348bb46eccf211414294ab3e',train=True,rank=16)
