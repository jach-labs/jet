...
REV='f446b82727be57da348bb46eccf211414294ab3e'
def main():
 trials=['lr3e-6','lr8e-6'];selection={}
 for t in trials:
 ...
 for name,adapter in [('released_v61',None),('selected_candidate',OUT/winner/'best')]:
  model,tok,_=load_model(str(ROOT/'releases/jet-v6.1'),REV,adapter=str(adapter) if adapter else None)
  report['test'][name]=evaluate(model,read_data(HERE/'test.jsonl',tok));print(name,json.dumps(report['test'][name]),flush=True)
  del model,tok;gc.collect();torch.cuda.empty_cache()
 base=report['test']['released_v61'];candidate=report['test']['selected_candidate']
 report['passes_final_guard']=selection[winner]['step']>0 and eligible(candidate,base) and candidate['objective']>base['objective']
 report['next_action']='Merge and validate this frozen candidate before any release.' if report['passes_final_guard'] else 'Keep Jet v6.1; no candidate passed the final improvement guard.'
 ...
 lines=['# Focused Jet v6.1 continuation','',f'Validation-selected trial: {winner}, step {selection[winner]["step"]}.','', '| Family | Released v6.1 | Candidate |','|---|---:|---:|']
