...
ROOT=Path(__file__).resolve().parents[2];HERE=Path(__file__).resolve().parent;P=ROOT/'releases/jet-v6.2';api=HfApi();repo='michaljach/jet'
...
assert head=='f446b82727be57da348bb46eccf211414294ab3e',f'Remote changed: {head}'
parent=json.loads((ROOT/'releases/jet-v6.1/release-manifest.json').read_text())['files']
...
api.create_tag(repo,tag='v6.1.0',revision=head,tag_message='Previous Jet full merged model before v6.2 continuation',exist_ok=True)
...
commit=api.create_commit(repo_id=repo,operations=ops,parent_commit=head,commit_message='Release Jet v6.2: full merged focused continuation and 16K runtime',num_threads=2)
api.create_tag(repo,tag='v6.2.0',revision=commit.oid,tag_message='Jet full merged BF16 continuation, selected step 250',exist_ok=True)
...
receipt={'repo_id':repo,'revision':commit.oid,'tag':'v6.2.0','previous_revision':head,'previous_tag':'v6.1.0','format':'full merged BF16 model','remote_large_file_hashes_verified':True,'url':'https://huggingface.co/michaljach/jet'}
