...
def run(name,args):
 with (LOGS/(name+'.log')).open('a') as log:
  process=subprocess.Popen([sys.executable,'-u',*map(str,args)],cwd=ROOT,stdout=log,stderr=subprocess.STDOUT,stdin=subprocess.DEVNULL)
...
try:
 ...
 for name,expected in json.loads((base/'release-manifest.json').read_text())['files'].items():
  if name.endswith('.safetensors'):
   with (base/name).open('rb') as f:assert hashlib.file_digest(f,'sha256').hexdigest()==expected
 run('smoke',[HERE/'train.py','--lr','5e-6','--out',OUT/'smoke','--smoke'])
 for name,lr in [('lr5e-6','5e-6'),('lr1e-5','1e-5')]:run(name,[HERE/'train.py','--lr',lr,'--out',OUT/name])
