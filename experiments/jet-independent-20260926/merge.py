...
ROOT=Path(__file__).resolve().parents[2];BASE=ROOT/'releases/jet-v6.2';ADAPTER=ROOT/'adapters/jet-rules-retention-20260926/anchor2/best';OUT=ROOT/'releases/jet-rules-candidate-20260926'
def sha(p):
...
(OUT/'merge-provenance.json').write_text(json.dumps({'name':'Jet','version':'unreleased-rule-candidate','base':'michaljach/jet','base_revision':'fbc3d2daa679e0d4bd9f99c9912b6496d5a41f0a','base_weights_sha256':{k:v for k,v in expected.items() if k.endswith('.safetensors')},'adapter_sha256':sha(ADAPTER/'adapter_model.safetensors'),'step':875,'merge':'Continue from the previously merged Jet v6.2 weights; add FP32 B@A correction and round to BF16. No original-Qwen reset.','merged_modules':len(merged),'tensors':len(mapping),'bytes':total},indent=2)+'\n')
