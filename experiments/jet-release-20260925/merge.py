...
ROOT=Path(__file__).resolve().parents[2];BASE=ROOT/'releases/jet-v6';ADAPTER=ROOT/'adapters/jet-targeted-20260924/best';OUT=ROOT/'releases/jet-v6.1'
def sha(p):
...
assert sha(ADAPTER/'adapter_model.safetensors')=='93e651d7aac0b2291d3a8dfbd6d6968a21bf5880a2ed6570e3cdd5ab9863c2cc'
...
(OUT/'merge-provenance.json').write_text(json.dumps({'name':'Jet','version':'v6.1.0','base':'michaljach/jet','base_revision':'e5b8f610ddb92ffaba596ae452bed32a9fef49ca','base_weights_sha256':{k:v for k,v in expected.items() if k.endswith('.safetensors')},'adapter_sha256':sha(ADAPTER/'adapter_model.safetensors'),'step':2000,'merge':'Continue from the previously merged Jet v6 weights; add FP32 B@A correction and round to BF16. No original-Qwen reset.','merged_modules':len(merged),'tensors':len(mapping),'bytes':total},indent=2)+'\n')
