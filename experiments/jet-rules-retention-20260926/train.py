...
from objective import anchored_loss
...
def main():
 p=argparse.ArgumentParser();p.add_argument('--lr',type=float,required=True);p.add_argument('--out',type=Path,required=True);p.add_argument('--anchor',type=float,required=True);p.add_argument('--smoke',action='store_true');args=p.parse_args()
 ...
 cache=json.loads((HERE/'teacher-logits.json').read_text());assert cache['train_sha256']==hashlib.sha256((HERE/'train.jsonl').read_bytes()).hexdigest()
 teachers={r['key']:r['logits'] for r in cache['rows']}
 for ex in train:
  key=hashlib.sha256(json.dumps([ex['ids'],ex['labels']]).encode()).hexdigest();ex['teacher']=teachers[key]
 ...
 for step in range(1,steps+1):
  ...
  loss_sum=0
  for ex in batch:
   z=label_logits(model,ex);loss=anchored_loss(z,torch.tensor(ex['target']),ex['type']=='score',ex['family'],ex['teacher'],args.anchor)
  ...
  if not args.smoke and (step%125==0 or step==steps):
