"""Freeze a validation-selected winner, then evaluate its reused diagnostic holdout once."""
...
def main():
 trials=['anchor2','anchor6'];selection={}
 for t in trials:
 ...
 report['next_action']='Collect an independent holdout, then merge and validate before any release.' if report['passes_final_guard'] else 'Keep Jet v6.2; no candidate passed the final improvement guard.'
 ...
 lines+=['',report['next_action'],'','All holdouts are reused development diagnostics; an independent holdout is required before release. No official Decision Index score is produced.','',*report['limitations']]
