"""Continue from released Jet with a fresh correction adapter and retention guard."""
import argparse,hashlib,json,math,random,time
from pathlib import Path
import numpy as np
import torch
from qwen35_training import load_model,examples,label_logits,decision_loss,evaluate as evaluate_raw


def family(source):
    if 'code_' in source:return 'code'
    if 'stance' in source:return 'stance'
    if 'sarcasm' in source:return 'sarcasm'
    if 'product_relevance' in source:return 'relevance'
    if 'preference_transfer' in source:return 'preference'
    return 'retention'

def evaluate(model, data):
    overall, logits = evaluate_raw(model,data,collect=True)
    buckets={}
    for ex,z in zip(data,logits):
        k=family(ex['source']);logp=z.astype(np.float64)-float(np.max(z))
        logp-=np.log(np.exp(logp).sum())
        loss=float(-np.dot(np.array(ex['target']),logp));correct=int(np.argmax(z)==np.argmax(ex['target']))
        buckets.setdefault(k,[]).append((loss,correct))
    scores={k:{'n':len(v),'nll':float(np.mean([a for a,b in v])),'acc':float(np.mean([b for a,b in v]))} for k,v in buckets.items()}
    focus=[v['nll'] for k,v in scores.items() if k!='retention']
    selection=.7*float(np.mean(focus))+.3*scores['retention']['nll'] if focus and 'retention' in scores else overall['nll']
    return {**overall,'selection_nll':selection,'families':scores}
...
def main():
    ...
    args=p.parse_args()
    assert args.out.is_dir() if args.resume else not args.out.exists(),args.out
    args.out.mkdir(parents=True,exist_ok=args.resume);torch.set_num_threads(4)
    ...
    config.update(dtype='bfloat16',quantization=None,gradient_checkpointing=True,microbatch=1,example_weighting='uniform within the focused training mix',selection_rule='70% mean focus-family NLL + 30% retention NLL; retention accuracy must stay within 2 percentage points of initial')
    if args.resume:
        prior=json.loads((args.out/'training_config.json').read_text())
        for key in ['base','revision','epochs','lr','rank','accumulate','seed','data_sha256']:
            assert config[key]==prior[key],key
    else:
        (args.out/'training_config.json').write_text(json.dumps(config,indent=2)+'\n')
    model,tok,info=load_model(args.base,args.revision,adapter=str(args.out/'last') if args.resume else None,train=True,rank=args.rank)
    ...
    (args.out/('resume_config.json' if args.resume else 'training_config.json')).write_text(json.dumps(config,indent=2,default=str)+'\n')
    print(json.dumps(config,default=str),flush=True)
    history=args.out/'metrics.jsonl'
    if args.resume:
        checkpoint=torch.load(args.out/'optimizer.pt',map_location='cpu',weights_only=True)
        optimizer.load_state_dict(checkpoint['optimizer']);step=int(checkpoint['step'])
        resume_step=step
        records=[json.loads(line) for line in history.read_text().splitlines()]
        assert records[-1]['step']==step,'Checkpoint and metric history disagree'
        retention_floor=records[0]['families']['retention']['acc']-.02
        best=json.loads((args.out/'best-step.json').read_text())
        if 'torch_rng' in checkpoint:
            torch.set_rng_state(checkpoint['torch_rng']);torch.cuda.set_rng_state_all(checkpoint['cuda_rng'])
        print(json.dumps({'resumed_from_step':step,'total_steps':steps,'optimizer_restored':True,'rng_restored':'torch_rng' in checkpoint}),flush=True)
        del checkpoint
    else:
        resume_step=0
        best=evaluate(model,val);retention_floor=best['families'].get('retention',{}).get('acc',0)-.02
    ...
    for epoch in range(args.epochs):
        ...
        for start in range(0,len(order),args.accumulate):
            if epoch*math.ceil(len(train)/args.accumulate)+start//args.accumulate < resume_step:continue
            ...
            if step%args.eval_every==0 or step==steps:
                scores=evaluate(model,val);improved=scores['selection_nll']<best['selection_nll'] and scores['families'].get('retention',{}).get('acc',1)>=retention_floor
                ...
                torch.save({'step':step,'epoch':epoch,'optimizer':optimizer.state_dict(),'torch_rng':torch.get_rng_state(),'cuda_rng':torch.cuda.get_rng_state_all()},args.out/'optimizer.pt')
                if improved:
