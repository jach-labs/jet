...
def main() -> None:
    ...
    ap.add_argument("--init-adapter", type=Path, default=None,
                    help="warm-start adapter weights with a fresh optimizer and schedule")
    ...
    args = ap.parse_args()
    if args.init_adapter and (args.resume or args.start_step is not None):
        ap.error("--init-adapter cannot be combined with --resume or --start-step")
    if (args.out / "adapters.safetensors").exists():
        ap.error("output already contains an adapter; choose a new directory")
    ...
    if args.init_adapter:
        source_config = json.loads((args.init_adapter / "adapter_config.json").read_text())
        expected = {"rank": args.rank, "scale": args.lora_scale, "dropout": args.dropout}
        if source_config.get("lora_parameters") != expected:
    ...
    if args.init_adapter:
        if source_config.get("num_layers") != num_layers:
            ap.error("initial adapter layer count does not match")
        weights = mx.load(str(args.init_adapter / "adapters.safetensors"))
        expected_weights = dict(tree_flatten(model.trainable_parameters()))
        if weights.keys() != expected_weights.keys() or any(
            weights[k].shape != expected_weights[k].shape for k in expected_weights
        ):
            ap.error("initial adapter tensor names or shapes do not match")
        model.load_weights(list(weights.items()), strict=False)
        print(f"initialized weights from {args.init_adapter}; optimizer starts fresh", flush=True)
    if args.resume:
    ...
    # Keep the initial checkpoint eligible; a regression must not replace it merely
    # because no trained checkpoint improved the validation loss.
    save_adapter(model, args.out, adapter_config, optimizer)
    (args.out / "metrics.jsonl").write_text(json.dumps({"step": 0, **best,
        "improved": False, "initial": True}) + "\n")
        ...
        for batch in make_batches(train, args.max_batch_tokens, rng):
            ...
            if step % args.eval_every == 0 or step == total_steps:
                ...
                if improved:
                    ...
                    saved = True
    if not saved:
        print("validation never improved; retaining initial weights (final weights in last/)")
