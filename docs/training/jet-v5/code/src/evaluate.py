...
from model import DEFAULT_BASE_MODEL, DEFAULT_MAX_STATE_TOKENS, Jet
...
def main_eval() -> None:
    ...
    ap.add_argument("--max-state-tokens", type=int, default=DEFAULT_MAX_STATE_TOKENS)
    ap.add_argument("--json", type=Path, default=None, help="also write metrics as JSON")
    args = ap.parse_args()

    jet = Jet(adapter_base(args.adapter, args.base_model), args.adapter, args.max_state_tokens)
